...
More careful translation - only visible text, not IDs/attributes
"""
...
# Tool titles and descriptions for each file
TOOL_CONTENT = {
    # Color tools (15)
...
# Simple label translations that appear between tags
LABEL_TRANSLATIONS = {
    "C (0-100)": "س (0-100)",
    "M (0-100)": "م (0-100)",
    "Y (0-100)": "ص (0-100)",
...
def translate_visible_text(content):
    """Translate only visible text content, not HTML attributes or IDs"""
    # First, protect content inside certain tags
    protected = []
    ...
    content = re.sub(r'(<script>.*?</script>)', protect, content, flags=re.DOTALL)
    content = re.sub(r'(<script[^>]*>.*?</script>)', protect, content, flags=re.DOTALL)
    # Protect style content  
    content = re.sub(r'(<style>.*?</style>)', protect, content, flags=re.DOTALL)
    # Protect onclick and other event handlers temporarily
    content = re.sub(r'(on\w+="[^"]*")', protect, content)
    content = re.sub(r"(on\w+='[^']*')", protect, content)
    
    # Now translate labels inside <label> tags
    for en, ar in LABEL_TRANSLATIONS.items():
        # Translate content between <label> tags
        pattern = rf'(<label[^>]*>)([^<]*{re.escape(en)}[^<]*)(</label>)'
        content = re.sub(pattern, lambda m: m.group(1) + m.group(2).replace(en, ar) + m.group(3), content)
        # Also translate button text
        pattern = rf'(<button[^>]*>)([^<]*{re.escape(en)}[^<]*)(</button>)'
        content = re.sub(pattern, lambda m: m.group(1) + m.group(2).replace(en, ar) + m.group(3), content)
        # And option text
        pattern = rf'(<option[^>]*>)([^<]*{re.escape(en)}[^<]*)(</option>)'
        content = re.sub(pattern, lambda m: m.group(1) + m.group(2).replace(en, ar) + m.group(3), content)
        # And placeholder text
        pattern = rf'placeholder="([^"]*{re.escape(en)}[^"]*)"'
        content = re.sub(pattern, lambda m: f'placeholder="{m.group(1).replace(en, ar)}"', content)
    ...
    for i, val in enumerate(protected):
        content = content.replace(f"___PROTECTED_{i}___", val)
...
def create_arabic_version(en_path, ar_path):
    ...
    new_switch = f'''<div class="lang-switch">
    ...
    # Update footer text
    content = content.replace("Part of 205 Tools Collection", "جزء من مجموعة أدوات 205")
    content = content.replace(">205 Tools<", ">205 أدوات<")
    
    # Translate visible text
    content = translate_visible_text(content)
